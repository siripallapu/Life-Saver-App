...
sheet.cell(row=current_row +1 , column=5).value = "Emergency"

def oneone():
    current_row = sheet.max_row
    current_column = sheet.max_column
    sheet.cell(row=current_row, column=6).value = "Heart Attack"
    wb.save('C:\\Users\\USER\\Desktop\\lifesaver database.xlsx')
def twotwo():
    current_row = sheet.max_row
    current_column = sheet.max_column
    sheet.cell(row=current_row, column=6).value = "Brain complication"
    wb.save('C:\\Users\\USER\\Desktop\\lifesaver database.xlsx')
def threethree():
    current_row = sheet.max_row
    current_column = sheet.max_column
    sheet.cell(row=current_row, column=6).value = "Accident"
    wb.save('C:\\Users\\USER\\Desktop\\lifesaver database.xlsx')
def fourfour():
    current_row = sheet.max_row
    current_column = sheet.max_column
